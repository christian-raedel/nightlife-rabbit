...
Realm.prototype.subscribe = function(uri, opts, session) {
    var self = this;

    if (parser.isUri(uri) && _.isPlainObject(opts) && self.session(session)) {
        var topics = self.topics;

        if (!topics[uri]) {
            topics[uri] = new Topic(opts);
        }
