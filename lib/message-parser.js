...
function MessageParser(opts) {
    ...
    var template = new CPlate();

    template.registerFilter('type', function (value) {
        return self.getTypeKey(value);
    });

    template.registerFilter('uri', function (value) {
        if (self.isUri(value)) {
            return value;
        } else {
    ...
    template.registerFilter('dict', function (value) {
    ...
    template.registerFilter('list', function (value) {
        if (_.isArray(value)) {
            return value;
        } else {
            throw new TypeError('wamp.error.invalid_argument');
    ...
    template.registerFilter('id', function (value) {
        if (_.isNumber(value)) {
            return value;
        } else {
            throw new TypeError('wamp.error.invalid_argument');
    ...
    template.registerFilter('optional', function (value) {
    ...
    template.registerFilter('transformer', function (value) {
        try {
    ...
    template.registerFilter('finalizer', function (value) {
...
MessageParser.prototype.encode = function(type, opts) {
    ...
    return q.fcall(function () {
        ...
        if (_.isNumber(key) && _.isPlainObject(opts)) {
            opts.type = key;
            return self.template.format(self.TYPES[key].encode, opts);
        } else {
